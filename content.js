...
// Gemini sayfasından konuşma verilerini çıkarır
function konusmayiCikar() {
    ...
    tumTurnlar.forEach((turn) => { // her dönüşü işle
        ...
        if (etiketAdi === 'user-query') { // kullanıcı mesajı
            ...
            if (kaynakElement) { // element bulunduysa
                let temizMetin = kaynakElement.textContent?.trim() || ''; // ham metin al
                temizMetin = geminiPrefixTemizle(temizMetin); // prefix'ini temizle
                mesajlar.push({
                    rol: 'kullanici', // mesaj rolü
                    html: kaynakElement.innerHTML, // HTML içerik
                    metin: temizMetin // temizlenmiş düz metin
                });
            }
            ...
            if (kaynakElement) { // element bulunduysa
                mesajlar.push({
                    ...
                    html: kaynakElement.innerHTML, // HTML içerik
                    metin: kaynakElement.textContent?.trim() // düz metin
                });
            }
        }
    });
